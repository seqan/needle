...
void initialize_argument_parser(argument_parser & parser, cmd_arguments & args)
{
    ...
    parser.add_option(args.exp_file, 'x', "expression", "A tab seperated file containing expression information per "
                      ...
                      option_spec::DEFAULT,input_file_validator{{"tsv"}});
...
int do_search(BD bd, cmd_arguments & args)
{
    ...
    debug_stream << "IBF_" + std::to_string(expression[0])<< "\n";
    ...
    for (unsigned i = 0; i < expression.size(); i++)
    {
        if ((i > 0) && (expression[i] != expression[i-1]))
        {
            ...
            debug_stream << "IBF_" + std::to_string(expression[i])<< "\n";
            cereal::BinaryInputArchive iarchive{is};
            iarchive(bd);
        }
...
int main(int const argc, char const ** argv)
{
    ...
    argument_parser parser("needle-search", argc, argv);
    cmd_arguments args{};
